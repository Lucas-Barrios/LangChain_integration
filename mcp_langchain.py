...
async def demo_read_resources(client: MultiServerMCPClient) -> None:
    ...
    try:
        # Get the underlying session for the filesystem server
        session = client.sessions.get("filesystem")
        if session is None:
            print("  (No active session — skipping resource demo)")
            return
        ...
        print(f"  Note: resource listing returned: {type(exc).__name__}: {exc}")
...
async def demo_list_directory(agent: Any) -> None:
    """Agent uses the list_directory MCP tool to discover documents."""
    print("\n--- Phase 3: Agent Discovers Documents via MCP Tool ---")
    result = await agent.ainvoke({
        "messages": [HumanMessage(content=(
            "Use the list_directory tool to show me all files in the current directory. "
...
async def demo_analyze_document(agent: Any) -> None:
    """Agent reads and analyzes a specific document using read_file."""
    print("\n--- Phase 4: Agent Reads & Analyzes a Document ---")
    result = await agent.ainvoke({
        "messages": [HumanMessage(content=(
            "Read the file 'ai_trends_2025.txt' and give me a 3-bullet executive summary "
            "of the top AI trends, written for a non-technical business audience."
        ))]
    })
...
async def demo_search_content(agent: Any) -> None:
    """Agent uses search_files or read_file to find specific content."""
    print("\n--- Phase 6: Agent Searches for Specific Content ---")
    result = await agent.ainvoke({
        "messages": [HumanMessage(content=(
            "Search through all documents to find every mention of ROI, cost, or financial figures. "
            "List what you find and which file each came from."
        ))]
    })
...
async def main() -> None:
    ...
    # It launches the MCP server subprocess and keeps the session alive
    # for the duration of the block.
    async with MultiServerMCPClient(MCP_SERVER_CONFIG) as client:
        # Phase 1: Inspect available tools
        tools = await demo_list_tools(client)

        # Phase 2: Try reading MCP resources directly
        ...
        await demo_analyze_document(agent)
        await demo_cross_document_search(agent)
        await demo_search_content(agent)
